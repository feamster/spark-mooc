...
# ### **Part 1: Basic Recommendations**
# #### One way to recommend movies is to always recommend the movies with the highest average rating. In this part, we will use Spark to find the name, number of ratings, and the average rating of the 20 movies with the highest average rating and more than 500 reviews. We want to filter our movies with high ratings but fewer than 500 reviews because movies with few reviews may not have broad appeal to everyone.
...
movieIDsWithAvgRatingsRDD = (movieIDsWithRatingsRDD
                             .map(lambda l: getCountsAndAverages(l)))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda l: (l[1][1][1], l[1][0], l[1][1][0]))
                             )
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda l: (l[0], math.pow(l[1][1] - l[1][0], 2)))
                       )
...
trainingAvgRating = float(trainingRDD
                     .map(lambda l: l[2])
                     .reduce(lambda x, y: x + y)) / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = (testRDD
                 .map(lambda l: (l[0], l[1], trainingAvgRating)))
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda l: (0, l[0]))
                      .filter(lambda l: (l[1] not in map(lambda m: m[1], myRatedMovies)))
                      )
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda l: l[1][1] > 75)
                       .join(moviesRDD)
                       .map(lambda l: (l[1][0][0], l[1][1], l[1][0][1]))
                      )

...
hosts = (access_logs
        .map(lambda log: (log.host, 1)).cache())

#uniqueHosts = hosts.<FILL IN>

uniqueHostCount = (hosts
...
def avgPerHost(l):
    total_requests = len(l)
    hosts = set()
    for hostreq in l:
        hosts.add(hostreq)
    return int(total_requests) / int(len(hosts))
    

avgDailyReqPerHost = (sortedByDay
                      .map(lambda x: (x[0], avgPerHost(x[1])))
                      .cache())
...
daysWithAvg = (avgDailyReqPerHost
               .map(lambda x: x[0]).collect())

avgs = (avgDailyReqPerHost
...
badRecords = (access_logs
              .map(lambda log: ((log.endpoint, log.response_code, log.host, log.date_time), 1))
              .filter(lambda x: (x[0][1] == 404))
              .cache()
             )
...
errHostsTop25 = (badRecords
                 .map(lambda l: ((l[0][2]),1))
                 .reduceByKey(add)
...
daysWithErrors404 = errDateSorted.map(lambda e: e[0]).collect()
errors404ByDay = errDateSorted.map(lambda e: e[1]).collect()
...
hoursWithErrors404 = hourRecordsSorted.map(lambda e: e[0]).collect()
errors404ByHours = hourRecordsSorted.map(lambda e: e[1]).collect()

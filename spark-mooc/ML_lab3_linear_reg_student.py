...
parsedSamplePoints = parsePoint(str(rawData.take(1)[0]))
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = (rawData
                  .map(lambda l: parsePoint(l)))

        
onlyLabels = parsedDataInit.map(lambda l: l.label)
minYear = onlyLabels.takeOrdered(1, key=lambda x: x)[0]
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda l: LabeledPoint(l.label - minYear, l.features))
...
def calcRMSE(labelsAndPreds):
    ...
    return math.sqrt(labelsAndPreds
            .map(lambda l: squaredError(l[0],l[1]))
...
print parsedTrainData.take(1)
labelsAndPredsTrain = (parsedTrainData
                       .map(lambda l: (l.label, averageTrainYear)))

print labelsAndPredsTrain.take(5)

print calcRMSE(labelsAndPredsTrain)
...
labelsAndPredsVal = (parsedValData
                     .map(lambda l: (l.label, averageTrainYear)))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = (parsedTestData
                      .map(lambda l: (l.label, averageTrainYear)))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda l: gradientSummand(w, l)).reduce(lambda x, y: x + y)
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = (LinearRegressionWithSGD
              .train(parsedTrainData,
                     numIters,
                     alpha,
                     miniBatchFrac,
                     np.zeros(len(parsedTrainData.take(1)[0].features)),
                     reg,
                     regType,
                     useIntercept
                    )
...
labelsAndPreds = (parsedValData
                  .map(lambda lp: (lp.label, firstModel.predict(lp.features)))
                 )
...
def twoWayInteractions(lp):
    ...
    newfeatures = np.hstack((lp.features,twoway))

...
def simpleTokenize(string):
    ...
    oldlist = re.split(split_regex,string.lower())
    return filter(lambda x: len(x)>0, oldlist) 
...
def tokenize(string):
    ...
    """
    tokens_stop = simpleTokenize(string)
    return filter(lambda x: not x in stopwords, tokens_stop)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return (vendorRDD
            .takeOrdered(1,key=lambda l: -len(l[1])))
...
def tf(tokens):
    ...
    for t in tokens:
        tdict[t] = tdict[t] + 1 if t in tdict else 1
    
    tfd = dict(map(lambda (k,v): (k, v/float(len(tokens))), tdict.iteritems()))
...
def idfs(corpus):
    ...
    # for each item in the corpus, a list of unique tokens
    uniqueTokens = corpus.map(lambda l: (sorted(set(l[1]))))

    # ('token', 1) each time a token appears in a document
    tokenCountPairTuple = (uniqueTokens
                           .flatMap(lambda l: map(lambda x: (x,1), l))
                          )
...
def tfidf(tokens, idfs):
    ...
    # foreach token in tokens, map to tf-idf
    #    tfIdfDict = <FILL IN>

    tfIdfDict = dict(map(lambda (k,v): (k, v*idfs[k]), tfs.iteritems()))
...
commonTokens = (amazonInvPairsRDD
                .rightOuterJoin(googleInvPairsRDD)
                .filter(lambda l: (l[1][0] != None) and (l[1][1] != None))
                .map(lambda l: swap(l))
...
def fastCosineSimilarity(record):
    ...
    w1 = amazonWeightsBroadcast.value[amazonRec]
    w2 = googleWeightsBroadcast.value[googleRec]
